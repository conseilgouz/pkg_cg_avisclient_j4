...
class mod_cg_avisclientInstallerScript
{
	...
	private function postinstall_cleanup() {
		...
		$obsloteFiles = [sprintf("%s/modules/mod_%s/helper.php", JPATH_SITE, $this->extname), 
						 sprintf("%s/modules/mod_%s/sppagination.php", JPATH_SITE, $this->extname),
                         sprintf("%s/modules/mod_%s/mod_cg_avisclient.php", JPATH_SITE, $this->extname),];
		...
		$query = $db->getQuery(true)
			->delete('#__update_sites')
			->where($db->quoteName('location') . ' like "%conseilgouz.com/updates/cg_avis%"');
		$db->setQuery($query);
		$db->execute();
		// remove packag infos
